...
uint16_t apu::unit::Sweep::getTarget() {
  return *channel_period_ + ((*channel_period_) >> shift_count_) * (negate_ ? -1 : 1) - (is_ch_2_ ? 0 : 1);
}

uint8_t apu::unit::Sweep::getOutput(uint8_t input) {
...
void apu::unit::LengthCounter::load(uint8_t code) {
...
void apu::unit::LengthCounter::clock() {
  if (counter_ == 0 || halt_) {
    return;
  }
  counter_--;
